...
| — Shocks en vivo (decisión) | `core/agent/shocks.py`, `POST /shock` | ✅ Los 4 tipos, con efecto medible en la decisión siguiente |
| — Shocks en vivo (simulación) | `core/simulation/engine.py` | ✅ Un `surge` queda vigente sobre su zona y encarece las ofertas que nacen ahí |
| — Estrategia tier2 | `core/agent/strategy.py` | ✅ `ClaudeAdvisor` + modo degradado + recuperación |
| — Evaluación | `core/evaluation/`, `scripts/run_evaluation.py`, `scripts/calibrate.py` | ✅ 6 políticas, seeds disjuntas, CSV del template, barrido reproducible |
...
python3 scripts/calibrate.py                 # rehace los 5 barridos (solo TUNING)
```
...
| Duración del surge en el simulador | `backend/core/simulation/engine.py` | `SURGE_DURATION_MIN` |
| Las 16 zonas y su `demand_score` | `backend/core/models.py` | `ZoneMap._DEFAULT_ZONES` |
...
├── scripts/        # run_evaluation.py, record_responses.py, calibrate.py
└── tests/

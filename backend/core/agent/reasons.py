...
def heat_rule(riding_min: float, limit_min: float, start_hour: int, end_hour: int) -> str:
    return cap_words(
        f"Regla de calor ({start_hour:02d}:00-{end_hour:02d}:00): {riding_min:.0f} min de "
        f"manejo continuo rebasan el tope de {limit_min:.0f} min de esa franja. "
        f"No se acepta hasta descansar."
    )

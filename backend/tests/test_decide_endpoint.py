...
def test_decide_rejects_flagged_zone_11_at_night():
    ...
    assert "zona 11 marcada" in data["reason"].lower()
...
def test_decide_rejects_flagged_zone_2_at_night():
    ...
    assert data["decision"] == "SKIP"
    assert data["binding_constraint"] == "flagged_zone_night"
    assert "zona 2 marcada" in data["reason"].lower()
...
def test_decide_handles_unknown_zone_gracefully_and_reports_zone_known_false():
    ...
    assert inputs["zone_known"] is False
    assert inputs["zone_pickup_known"] is False
    # La zona de recogida no existe en nuestro catalogo: demanda neutral, ni
    # premia ni castiga. El score de dropoff NO tiene por que ser neutral --
    # esa zona si la conocemos, y es la que usa la economia.
    assert inputs["pickup_demand_score"] == pytest.approx(0.50)
    assert inputs["zone_dropoff_known"] is True
